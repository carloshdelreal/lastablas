...
function buildQuestions(tables: number[], from: number, to: number): Question[] {
  const qs: Question[] = []
...
function App(): JSX.Element {
  ...
  const submit = (): void => {
    ...
    const isCorrect = Number.isFinite(parsed) && parsed === current.multiplicand * current.multiplier
    const record: Answer = { question: current, given: answer.trim(), correct: Boolean(isCorrect) }
    ...
    <div className="min-h-full flex flex-col items-center px-4 py-8">
      <div className="w-full max-w-2xl">
        <header className="mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-300">Práctica de multiplicaciones con orden aleatorio y resumen final.</p>
        </header>
        {!started ? (
          ...
          <section className="space-y-4 bg-white/60 dark:bg-white/5 rounded-xl p-4 shadow-sm ring-1 ring-gray-200 dark:ring-white/10">
            <div className="flex items-baseline justify-between">
              <p className="text-sm text-gray-600 dark:text-gray-300">Progreso: {solvedUnique}/{totalQuestions}</p>
              <p className="text-sm">Aciertos: <span className="font-semibold text-emerald-600">{correctCount}</span></p>
            </div>
            ...
            {answers.length > 0 && (
              <div className="text-sm text-gray-600 dark:text-gray-300">
                Última respuesta: {answers[answers.length - 1].correct ? (
                  ...
                  {answers.map((a, idx) => {
                    ...
                    return (
                      <tr key={idx} className="border-t border-gray-200 dark:border-white/10">
                        <td className="py-1 pr-4">{a.question.multiplicand} × {a.question.multiplier} = {correct}</td>
                        <td className="py-1 pr-4">{a.given || '(vacío)'}</td>
                        <td className="py-1">{ok ? (
      ...
      <footer className="mt-10 text-xs text-gray-500">Hecho con React + Vite + Tailwind</footer>
    </div>
  )
}
